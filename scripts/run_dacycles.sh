    ...
    for isec in $(seq $start_sec $intvl_sec $end_sec ); do
        ...
        if [[ " ${jobs[*]} " =~ " mpas " ]]; then
            ...
            if [[ $dorun == true ]]; then
                ...
                check_and_resubmit "fcst" $dawrkdir $npefcst $mpas_jobscript 2
            fi
        fi
...
eventtime="1500"
initdatetime="$eventdate$eventtime"
...
if [[ $machine == "Jet" ]]; then
    ...
    npefilter=48   ; nnodes_ics=$((  npefilter/ncores_filter   ))
    npefcst=48     ; nnodes_fcst=$(( npefcst/ncores_fcst ))
