...
function run_init {
    ...
    if [[ -f $wrkdir/running.ics || -f $wrkdir/done.ics || -f $wrkdir/queue.ics ]]; then
        ...
        if [[ ! -f "${FIXDIR}/$domname.graph.info.part.${npeics}" ]]; then
            split_graph "${gpmetis}" "${domname}.graph.info" "${npeics}" "${FIXDIR}" "$dorun" "$verb"
        fi
...
function run_mpas {
    ...
    if [[ -f $wrkdir/running.fcst${appendhour_str} || -f $wrkdir/done.fcst${appendhour_str} || -f $wrkdir/queue.fcst${appendhour_str} ]]; then
        ...
        if [[ ! -f "${FIXDIR}/$domname.graph.info.part.${npefcst}" ]]; then
            split_graph "${gpmetis}" "${domname}.graph.info" "${npefcst}" "${FIXDIR}" "$dorun" "$verb"
        fi
...
[[ -v args["WORKDIR"] ]] && WORKDIR=${args["WORKDIR"]} || WORKDIR="${mpasdir}/run_dirs/conus"
...
if [[ $machine == "Jet" ]]; then
    ...
    ncores_post=32
    partition="regular"
    claim_cpu="ncpus=${ncores_fcst}"
    claim_cpu_ics="ncpus=${ncores_ics}"
    partition_static="regular"
    static_cpu="ncpus=${ncores_post}"
    partition_upp="regular"

    npeics=768
    nnodes_ics=$((npeics / ncores_ics))
