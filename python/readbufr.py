...
def read_snd(filename,outtable=None):
    ...
    while bufr.advance() == 0:
        if bufr.msg_type not in msgtypes:
            msgtypes[bufr.msg_type] = 1
        else:
            msgtypes[bufr.msg_type] += 1
    ...
    print(f"Total messages in the file: {nmessages}")
...
if __name__ == "__main__":
    #filename = '/work/rt_obs/SBUFR/2023/03/31/rap.2023033121.prepbufr.tm00'
    outtable = 'rap.table'

    if len(sys.argv) < 2:
        raise SystemExit('readbufr <input prepbufr>')

    # input and output file names from command line args.
    filename = sys.argv[1]

    stations = read_snd(filename,outtable)
